...
- **先查 [jd-bank/_index.local.md](jd-bank/_index.local.md)**（没有再查 `_index.md`）— 同公司同岗位半年内分析过，告诉用户"上次解码过，要不要直接复用"。
...
2. 按 [frameworks/offer-strategy-report.md](frameworks/offer-strategy-report.md) 的规格 + [examples/offer-strategy-template.html](examples/offer-strategy-template.html) 的骨架组装一份 10 节报告（TL;DR · 关键指标 · 1-10）。
   - ⛔ **品牌 footer 是强制项，不是装饰。** 报告结尾必须**原样**包含 `<footer>` 里的 `JD SKILL.` brand mark + `by Rick` 副标题 + **微信小程序 / 小红书**两个按钮（含对应 CSS：`.brand-block` / `.brand-mark` / `.socials` / `.wechat` / `.xhs` / `.foot-meta`）。这是作者署名，**无论报告多长、数据多少都不许删或简化**。生成时直接从 [frameworks/offer-strategy-report.md](frameworks/offer-strategy-report.md) 末尾「📌 强制 Footer 区块」整段抄过去。**禁止**出现 Dreameryanyan / LinkedIn / X（Twitter）图标。
3. 写到 `./output/offer-strategy-<company>-<role>-<YYYYMM>.html`。
   - 写完后**自检一次**：文件里必须能搜到 `Rick`、`brand-mark`、`小程序://Bonjour`、`xhslink.com` 四个关键词。缺任何一个 = footer 被丢了，必须补回再继续。
4. **自动打开**：跑 `open "<完整路径>"`（macOS）/ `xdg-open` (Linux) / `start` (Windows)，让 HTML 直接在浏览器弹出来。
5. **同步 [jd-bank/](jd-bank/)**：按 [jd-bank/_jd-template.md](jd-bank/_jd-template.md) 写一份 `<slug>.md`，并登记到 **[jd-bank/_index.local.md](jd-bank/_index.local.md)**（没有就新建）。`_index.md` 是仓库空壳，**不要往里头写候选人姓名 / 公司投递等个人信息**（避免误提交到公开仓库）。文件末尾加一行 `> 📊 HTML 报告：./output/offer-strategy-<slug>.html`。

最后一句收尾：

> "报告已生成并在浏览器打开 ✅
> · 📊 `./output/offer-strategy-<slug>.html`
...
- [jd-bank/_index.md](jd-bank/_index.md) 是仓库空壳反查表结构。**个人登记写到 `jd-bank/_index.local.md`（gitignore）**。查库时先读 `_index.local.md`，没有再看 `_index.md`。**每次 Step 3 生成报告都要同步 `_index.local.md`，不要把个人信息写进会被 push 的 `_index.md`。**
